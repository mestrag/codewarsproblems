import org.junit.Test;

import static org.junit.Assert.assertArrayEquals;

public class RowOfOddTriangleTest {
    @Test
    public void test_1() {
        assertArrayEquals(new long[]{1}, RowOfOddTriangle.oddRow(1));
    }

    @Test
    public void test_2() {
        assertArrayEquals(new long[]{3, 5}, RowOfOddTriangle.oddRow(2));
    }

    @Test
    public void test_13() {
        assertArrayEquals(new long[]{
                157, 159, 161, 163, 165, 167, 169, 171, 173, 175, 177, 179, 181
        }, RowOfOddTriangle.oddRow(13));
    }

    @Test
    public void test_19() {
        assertArrayEquals(new long[]{
                343, 345, 347, 349, 351, 353, 355, 357, 359, 361, 363, 365, 367, 369, 371, 373, 375, 377, 379
        }, RowOfOddTriangle.oddRow(19));
    }

    @Test
    public void test_41() {
        assertArrayEquals(new long[]{
                1641, 1643, 1645, 1647, 1649, 1651, 1653, 1655, 1657, 1659, 1661, 1663, 1665, 1667, 1669,
                1671, 1673, 1675, 1677, 1679, 1681, 1683, 1685, 1687, 1689, 1691, 1693, 1695, 1697, 1699,
                1701, 1703, 1705, 1707, 1709, 1711, 1713, 1715, 1717, 1719, 1721
        }, RowOfOddTriangle.oddRow(41));
    }

}